...
class GlisterStochasticReg():
    ...
    def random_greedy_train_model_online_taylor(self, start_rand_idxs):
        ...
        for i in range(self.num_epochs):
            ...
            subset_idxs, grads_idxs = setf_model.naive_greedy_max(budget = int(self.lam * self.bud), theta_init = clone_dict)
            rem_idxs = list(set(random_idxs).difference(set(subset_idxs)))
            subset_idxs.extend(list(np.random.choice(rem_idxs, size=int((1 - self.lam) * self.bud), replace=True)))
            idxs = subset_idxs
