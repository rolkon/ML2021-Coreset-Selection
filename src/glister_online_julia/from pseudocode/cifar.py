...
def GreedyDSS(U, Val, theta_prev, eta, k, r, lambd, R, sel):
  ...
  while t < r:
    if sel == "naive_greedy":
      ...
      random_idxs = random.sample(total_idxs, round(len(U)*0.01))
      V = [U[i] for i in random_idxs]
    ...
    for e in V:
      ...
      for i in range(len(S)):
        grad_s, _ = compute_grad_train(S[i], theta_t_e input_dim, n_classes)
...
experimental_setting(train_ds, val_ds, test_dataset, torch.randn(1, 2560), 0.05, 4500, 100, 0, 0, "naive_greedy", 32*32*3, 10, 100, 20, 128, CifarNet()) # ~10% of data

print("Approximately 30% of data")
experimental_setting(train_ds, val_ds, test_dataset, torch.randn(1, 2560), 0.05, 15000, 300, 0, 0, "naive_greedy", 32*32*3, 10, 100, 20, 128, CifarNet()) # ~30% of data

print("Approximately 50% of data")
experimental_setting(train_ds, val_ds, test_dataset, torch.randn(1, 2560), 0.05, 20000, 500, 0, 0, "naive_greedy", 32*32*3, 10, 100, 20, 128, CifarNet()) # ~50% of data