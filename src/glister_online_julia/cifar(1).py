...
def GreedyDSS(U, Val, theta_prev, eta, k, r, lambd, R, sel):
  ...
  while t < r:
    if sel == "naive_greedy":
      ...
      random_indices = random.sample(total_idxs, round(len(U)*0.001))
      V = [U[i] for i in random_indices]
...
def experimental_setting(train_data, val_data, test_data, theta_prev, eta, k, r, lambd, R, sel, input_dim, n_classes, T, L, batch_size, network):
  ...
  train(100, net, criterion, optimizer, train_loader, test_loader, verbose=True)
  ...
  train(100, net_2, criterion, optimizer, random_samp_loader, test_loader, verbose=True)
...
experimental_setting(train_ds, val_ds, test_dataset, torch.randn(1, 2560), 0.05, 4500, 100, 0, 0, "stochastic_greedy", 32*32*3, 10, 100, 20, 128, CifarNet()) # ~10% of data

print("Approximately 30% of data")
experimental_setting(train_ds, val_ds, test_dataset, torch.randn(1, 2560), 0.05, 15000, 100, 0, 0, "stochastic_greedy", 32*32*3, 10, 100, 20, 128, CifarNet()) # ~30% of data

print("Approximately 50% of data")
experimental_setting(train_ds, val_ds, test_dataset, torch.randn(1, 2560), 0.05, 20000, 100, 0, 0, "stochastic_greedy", 32*32*3, 10, 100, 20, 128, CifarNet()) # ~50% of data